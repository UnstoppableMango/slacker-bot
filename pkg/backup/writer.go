...
func (w *Writer) Write(ctx context.Context, b *pb.ServerBackup) error {
	log := log.FromContext(ctx).With("guild", b.GetGuild().GetName())

	name := fmt.Sprintf("%s.binpb", b.GetGuild().GetName())
	log.Info("writing backup", "file", name)
	err := ihfs.WriteReader(w.fs, name, strings.NewReader(b.String()), os.ModePerm)
	if err != nil {
		return err
	}

	for _, u := range backupURLs(b) {
		if err := w.persistURL(ctx, u); err != nil {
			return err
		}
	}
...
func (w *Writer) persistURL(ctx context.Context, raw string) error {
	if raw == "" {
		return nil
	}

	log := log.FromContext(ctx).With("url", raw)
	...
	log.Debug("writing asset", "path", path)
...
func Write(ctx context.Context, b *pb.ServerBackup, fsys ihfs.FS) error {
	return NewWriter(fsys).Write(ctx, b)
}
